...
Public Class Sale
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        If String.IsNullOrWhiteSpace(TxtCustomerName.Text) OrElse
            ...
            String.IsNullOrWhiteSpace(TxtInvoiceNo.Text) OrElse
        ...
        Using cn As New OleDbConnection(connectionString)
            Try
                cn.Open()
                Dim cmd As New OleDbCommand("INSERT INTO SALE (Customer_name,Phone_number,Address,Invoice_no, [Date], Quantity, Amount) VALUES (?, ?, ?, ?, ?, ?, ?)", cn)
                cmd.Parameters.AddWithValue("Invoice_no", TxtCustomerName.Text)
                cmd.Parameters.AddWithValue("Invoice_no", TxtPhoneNumber.Text)
                cmd.Parameters.AddWithValue("Invoice_no", TxtAddress.Text)
                cmd.Parameters.AddWithValue("Invoice_no", TxtInvoiceNo.Text)
                cmd.Parameters.AddWithValue("Date", TxtDate.Text)
                cmd.Parameters.AddWithValue("Quantity", TxtQuantity.Text)
                cmd.Parameters.AddWithValue("Amount", TxtGrandTotal.Text)
                cmd.ExecuteNonQuery()
    ...
    Private Sub BtnPlus_Click(sender As Object, e As EventArgs) Handles BtnPlus.Click
        ...
        If String.IsNullOrWhiteSpace(TxtCustomerName.Text) OrElse String.IsNullOrWhiteSpace(TxtPhoneNumber.Text) OrElse String.IsNullOrWhiteSpace(TxtAddress.Text) OrElse String.IsNullOrWhiteSpace(TxtInvoiceNo.Text) OrElse String.IsNullOrWhiteSpace(TxtDate.Text) Then
            MsgBox("Please fill in Empty Field")
            Return
        End If
        ...
        Dim rowIndex As Integer = DataGridViewSale.Rows.Add()

        ' Set the values of the cells in the new row
        DataGridViewSale.Rows(rowIndex).Cells(0).Value = rowIndex + 1
        DataGridViewSale.Rows(rowIndex).Cells(1).Value = value1
        DataGridViewSale.Rows(rowIndex).Cells(2).Value = value2
        DataGridViewSale.Rows(rowIndex).Cells(3).Value = value3
        DataGridViewSale.Rows(rowIndex).Cells(4).Value = value4
        DataGridViewSale.Rows(rowIndex).Cells(5).Value = value5
        DataGridViewSale.Rows(rowIndex).Cells(6).Value = amount

        Dim Total As Decimal = 0
        Dim Qty As Decimal = 0

        For Each row As DataGridViewRow In DataGridViewSale.Rows
    ...
    Private Sub Btnless_Click(sender As Object, e As EventArgs) Handles Btnless.Click
        ...
        If DataGridViewSale.SelectedRows.Count > 0 Then
            ' Get the selected row index
            Dim rowIndex As Integer = DataGridViewSale.SelectedRows(0).Index

            ' Remove the selected row
            DataGridViewSale.Rows.RemoveAt(rowIndex)
